...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='/Users/eliasaoundurand/Projets/CausaL/psychopy_exps/vb_task_lastrun.py',
...
avec = visual.TextStim(win=win, name='avec',
    text="Avec"+"\n flèche gauche",
    font='Open Sans',
    units='cm', pos=(-5, 0), height=1.0, wrapWidth=None, ori=0.0, 
    color=[-1.0000, -1.0000, -1.0000], colorSpace='rgb', opacity=1.0, 
    languageStyle='LTR',
    depth=-3.0);
sans = visual.TextStim(win=win, name='sans',
    text="Sans"+"\n flèche droite",
    font='Open Sans',
    units='cm', pos=(5, 0), height=1.0, wrapWidth=None, ori=0.0, 
...
image_outcome = visual.ImageStim(
    win=win,
    name='image_outcome', units='cm', 
...
for thisTrial in trials:
    ...
    _key_resp_allKeys = []
    # keep track of which components have finished
    trialComponents = [question_mark, key_resp, avec, sans]
    ...
    while continueRoutine:
        ...
        if question_mark.status == NOT_STARTED and tThisFlip >= 2.0-frameTolerance:
            # keep track of start time/frame for later
        ...
        # *avec* updates
        if avec.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
            # keep track of start time/frame for later
            avec.frameNStart = frameN  # exact frame index
            avec.tStart = t  # local t and not account for scr refresh
            avec.tStartRefresh = tThisFlipGlobal  # on global time
            win.timeOnFlip(avec, 'tStartRefresh')  # time at next scr refresh
            # add timestamp to datafile
            thisExp.timestampOnFlip(win, 'avec.started')
            avec.setAutoDraw(True)
        
        # *sans* updates
        if sans.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
            # keep track of start time/frame for later
            sans.frameNStart = frameN  # exact frame index
            sans.tStart = t  # local t and not account for scr refresh
            sans.tStartRefresh = tThisFlipGlobal  # on global time
            win.timeOnFlip(sans, 'tStartRefresh')  # time at next scr refresh
            # add timestamp to datafile
            thisExp.timestampOnFlip(win, 'sans.started')
            sans.setAutoDraw(True)
    ...
    if key_resp.corr:
        # c'est à dire si left est actionné en l'occurence play
        # on a Bern(p)
        outcome = np.random.binomial(1, bern_param_play, size=1)
        if outcome == 1 :
            fb_text = 'images/Win.png'
        else: 
            fb_text = 'images/Lose.png'
    else:
        # ie si right est actionné
        outcome = np.random.bernoulli(1, bern_param_not_play, size=1)
        if outcome == 1:
            fb_text = 'images/Win.png'
        else:
            fb_text = 'images/Lose.png'
    ...
    # keep track of which components have finished
    outcomeComponents = [image_outcome]
    ...
    while continueRoutine and routineTimer.getTime() < 2.0:
        ...
        # *image_outcome* updates
        if image_outcome.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
            # keep track of start time/frame for later
            image_outcome.frameNStart = frameN  # exact frame index
            image_outcome.tStart = t  # local t and not account for scr refresh
            image_outcome.tStartRefresh = tThisFlipGlobal  # on global time
            win.timeOnFlip(image_outcome, 'tStartRefresh')  # time at next scr refresh
            # add timestamp to datafile
            thisExp.timestampOnFlip(win, 'image_outcome.started')
            image_outcome.setAutoDraw(True)
        if image_outcome.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > image_outcome.tStartRefresh + 2.0-frameTolerance:
                # keep track of stop time/frame for later
                image_outcome.tStop = t  # not accounting for scr refresh
                image_outcome.frameNStop = frameN  # exact frame index
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'image_outcome.stopped')
                image_outcome.setAutoDraw(False)
        if image_outcome.status == STARTED:  # only update if drawing
            image_outcome.setImage(fb_text, log=False)
